// src/components/Administrador/Mesa/MesaForm.tsx
...
export default function MesaForm({ onSubmit }: MesaFormProps) {
  ...
  useEffect(() => {
    // Cargar restaurantes
    axios
      .get(`${API_BASE_URL}/restaurantes`)
      .then((res) => setRestaurantes(res.data))
      .catch((err) => console.error("Error al cargar restaurantes:", err));

    // Cargar sucursales
    axios
      .get(`${API_BASE_URL}/sucursales`)
      .then((res) => setSucursales(res.data))
      .catch((err) => console.error("Error al cargar sucursales:", err));
  }, []);

  // Filtrar sucursales por restaurante si así lo deseas
  const sucursalesFiltradas =
    selectedRestaurante === ""
      ? sucursales
      : sucursales.filter((s) => s.restaurante_id === Number(selectedRestaurante));
      ...
      >
        ...
        <FormControl fullWidth>
          ...
          <Select
            ...
            onChange={(e) => setSelectedRestaurante(e.target.value as number)}
          >
            <MenuItem value="">
              <em>-- Seleccionar --</em>
            </MenuItem>
        ...
        {/* Seleccionar Sucursal (filtrada) */}
        <FormControl fullWidth>
          <InputLabel>Sucursal</InputLabel>
          <Select
            value={selectedSucursal}
            onChange={(e) => setSelectedSucursal(e.target.value as number)}
          >
            <MenuItem value="">
              <em>-- Seleccionar --</em>
            </MenuItem>
